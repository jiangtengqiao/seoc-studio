# 修复 RevealLens：错位根因是 lens-dash 旋转动画覆盖了 Tailwind 位移 transform；
# 跟不上根因是位置插值平滑。重写为：单一容器 transform（位移+缩放合一）、
# 子元素纯 flex 居中无 transform、位置零延迟直跟、仅缩放与空闲游走做插值。
import io
...
io.open(p, 'w', encoding='utf-8', newline='').write(s + tail)
print('RevealLens fixed')
