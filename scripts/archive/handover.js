  ...
  return [
    ...
    h.table(
      ...
      [
        ...
        ['ai_models', 'AI 模型注册表（管理员维护）', 'provider 限定 4 厂商'],
        ['ai_credits', '用户研点余额', '注册触发器自动建行'],
        ['ai_transactions', '研点流水', 'type 限定 5 类'],
        ['ai_usage_logs', '每条消息 token 用量与扣费', '含 interrupted 中断标记'],
        ['ai_api_keys', '用户 API 密钥（SHA-256 哈希存储）', 'key_hash 唯一'],
        ['ai_context_configs', 'AI 上下文配置（预留二次开发）', '公开读'],
        ['ai_topup_orders', '研点充值订单（人工确认制）', 'pending→confirmed 触发加余额'],
      ],
    ...
    h.bullet('supabase/ai-platform.sql：AI 研智助手平台七张表（ai_models/ai_credits/ai_transactions/ai_usage_logs/ai_api_keys/ai_context_configs/ai_topup_orders）+ RLS + 触发器 + 初始模型数据（含豆包/千问/智谱/DeepSeek 共 7 个模型）。v9.4 新增，需执行一次，可重复执行。'),
    h.p('脚本均设计为可重复执行（drop policy if exists 前置）。执行入口：Supabase 控制台 SQL Editor。'),
    h.h2('8.4 Edge Functions'),
    h.p('supabase/functions/ 下分为两类：一，发信函数（send-code/verify-code/register-user/reset-password），调用 QQ 邮箱 SMTP（jiangtengqiao@qq.com，授权码存于 Function 环境变量）发送注册与重置验证码邮件；二，AI 函数（ai-chat/ai-api-proxy），调用豆包/千问/智谱/DeepSeek 四厂商 OpenAI 兼容接口，支持流式 SSE 响应、token 实时计费、余额不足中断。厂商 API Key 存于 Edge Functions Secrets（DOUBAO_API_KEY/QWEN_API_KEY/ZHIPU_API_KEY/DEEPSEEK_API_KEY），前端绝不接触。ai-api-proxy 提供 OpenAI 兼容接口供用户以 API 密钥调用。'),
  ];
}
