...
- [x] v5 商业闭环与质量升级（2026-08-09 晚间）
  - 全站价格整体下调约 20% 至 23%，探索式 8 个子项目合计 1318 元，总期刊包 1159 元
  - 字数承诺升级：订阅式每章不少于 8000 字，专研式每期不少于 10000 字，探索式每期 8000 至 15000 字
  - 产品页真实购买面板接入支付宝收款码、付款信息提交、purchases 待确认订单、已开通与核对中状态
  - 探索门类页新增总期刊包购买卡片，总包确认后覆盖全部 8 个探索式子项目
  - 用户中心显示总包名称与待确认申请；管理端显示付款信息并可确认或驳回
  - Supabase RLS 已修复，exploration-bundle 可读取全部 exp-python-* 期刊与资料
  - 管理端发布内容时按产品类别校验最低字数，防止短正文上线
  - 能力评估题库由 78 题扩充至 150 题，六个维度继续保持动态抽题
  - python-origin 第 1 章重写为 13584 字详细标杆，后续文章按同等深度补齐
  - send-code 邮件函数已同步新总包价格并重新部署

## v7（2026-08-10）
- 全站 i18n：9 种界面语言（简中/繁中/英/日/韩/法/德/西/俄），localStorage 持久化，导航/页脚/首页/阅读器全覆盖
- 主题系统：浅色/深色/跟随系统三态切换，Tailwind class 策略 + 全局 .dark 覆盖层
- 阅读页趣味交互：快问快答浮窗、划重点（localStorage 持久化）、阅读里程碑提示、研读计时、章节上下期导航（含标题）与目录速览
- 首页底部扩充：内容体量总览、适用人群、站内功能导览、版权保护说明、联系与异议通道；修复回馈区块重复 bug；FAQ 扩至 10 条
- 字数承诺整体上抬：订阅每章≥22000、专研每期≥26000、探索每期 22000-30000/26000-30000，products.ts、法律文本、Admin 校验全部同步
- 提交 0a90130 已上线
- 本地交付：SEOC学习资料大纲与写作要求.md（102 章/期，工作区根目录，不上线）

## v7.1（2026-08-10）
- 阅读页新增本篇大纲导航：桌面端右侧粘性栏（scrollspy 高亮当前节+进度条）、移动端顶部折叠面板；从 Markdown 提取 h2/h3 生成锚点，点击平滑跳转
- 排版优化：正文行高/字距/标题节奏、代码块阴影、列表 marker 着色、h4/hr/strong 样式补齐
