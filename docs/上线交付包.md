...
> 上线：① 在 Supabase SQL Editor 依次执行下方五段 SQL → ② 重新部署两个 Edge Function → ③ git push 触发 GitHub Pages 自动发布。
> 已执行过旧版本的，只需执行新增部分（当前新增：SQL ⑤ fix-v11）。
...
## 第 6 步：重新部署 Edge Function（首次或改动后需要）

```bash
npx supabase functions deploy ai-chat
npx supabase functions deploy ai-api-proxy
```
或把 `supabase/functions/.deploy/ai-chat-inline.ts` 与 `supabase/functions/.deploy/ai-api-proxy-inline.ts` 贴到控制台对应函数。
...
1. 聊天页：气泡排版正常、流式光标跟在文字后、向上翻阅不闪跳、停止生成
2. 通知：已读后红点立即消失；支持单条删除与「删除已读」
3. 余额：管理端确认充值后 30 秒内右上角余额自动刷新
4. Admin → AI 平台管理 → 模型定价：被禁用模型仍显示，可重新启用
5. DeepSeek 模型为 V4 Flash / V4 Pro；订单「我已支付」+ 24 小时确认
6. 充值：固定三档 + 自定义正整数金额（1 元 = 1000 研点）
7. 免费额度：当天首次发消息自动获得模型免费次数，不再误报余额不足
