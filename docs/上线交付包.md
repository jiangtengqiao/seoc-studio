...
> 三步上线：① 在 Supabase SQL Editor 执行下方两段 SQL → ② 重新部署两个 Edge Function → ③ git push 触发 GitHub Pages 自动发布。
> 全部改动已本地提交，推送前请确认。

## 第 1 步：SQL ①（fix-v7 计费与权限加固）

打开 Supabase 控制台 → SQL Editor → New query → 粘贴执行（可重复执行）：
...
继续在同一 SQL Editor 粘贴执行（必须在 fix-v7 之后）：
...
1. 聊天页：会话侧边栏、停止生成按钮、余额中断提示
2. 研点页：充值/会员下单 → 30 分钟倒计时 → 可取消
3. Admin → AI 平台管理：全站统计 / 在线改价 / 用户管理（研点、会员、封禁）/ 内容审核敏感词
4. 顶栏铃铛：订单确认/驳回、会员到期、封禁均收到站内通知
5. API：`GET /v1/models`、max_tokens 透传、超限 429、敏感词 400

## 文件地图（改动清单）

- `supabase/fix-v7-ai-hardening.sql`、`supabase/fix-v8-ai-admin.sql` — 本次 SQL
- `supabase/functions/ai-chat/index.ts`、`supabase/functions/ai-api-proxy/index.ts`、`supabase/functions/_shared/ai-providers.ts` — Edge Function
- `supabase/functions/.deploy/*-inline.ts` — 控制台部署版（已同步）
- `src/lib/ai.ts`、`src/pages/AIChat.tsx`、`src/pages/AICredits.tsx`、`src/pages/AIApiKeys.tsx`、`src/pages/Admin.tsx`、`src/components/AIAdminPanel.tsx`、`src/pages/Notifications.tsx`、`src/components/Layout.tsx`、`src/App.tsx`、`src/lib/i18n.tsx`、`src/lib/types.ts`、`src/lib/auth.tsx` — 前端
- `docs/ai-hardening-deploy.md` — 完整部署说明
