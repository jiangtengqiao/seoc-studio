# 研智助手 v7+v8+v9+v10 · 上线交付包

> 上线：① 在 Supabase SQL Editor 依次执行下方四段 SQL → ② 重新部署两个 Edge Function → ③ git push 触发 GitHub Pages 自动发布。
> 已执行过旧版本的，只需执行新增部分（当前新增：SQL ④ fix-v10）。
...
## 第 5 步：重新部署 Edge Function（仅首次或改动后需要）
...
1. 聊天页：气泡排版正常（不一行一个字）、流式光标跟在文字后、字体适中、向上翻阅不闪跳
2. 通知：已读后红点立即消失；支持单条删除与「删除已读」
3. 余额：管理端确认充值后 30 秒内（或切回页面时）右上角余额自动刷新
4. Admin → AI 平台管理 → 模型定价：被禁用的模型仍显示在列表中，可重新启用
5. DeepSeek 模型为 V4 Flash / V4 Pro；订单「我已支付」+ 24 小时确认；会员新价格
